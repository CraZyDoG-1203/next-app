...
const Page = () => {
  return (
    <div style={{ fontFamily: 'Arial, sans-serif', lineHeight: '1.6', margin: '20px' }}>
      ...
      <ul>
        <li><b>可擴展性：</b> REST 的每一項請求都是獨立的（又稱無狀態），伺服器不需要保留過去用戶端的請求，且 REST 會最佳化用戶端與伺服器之間的互動，從而降低伺服器負載。</li>
        <li style={{ textIndent: '5em' }}>上述所說的都可以支援可擴展性，nbsp減少通訊瓶頸的發生。</li>
        <li><b>靈活性：</b> RESTful Web 支援用戶端與伺服器完全分離，這項服務簡化並去耦合各伺服器元件，讓每一部分都可以獨立演進。因用戶端與伺服器是完全分離，平台的變更不會影響用戶端的應用程式。</li>
        <li><b>獨立性：</b> 可以使用任意程式語言來設計用戶端和伺服器的應用，而不會影響 API 的設計。此外，還可以在不影響通訊的情況下變更任一端的基礎技術。</li>
      </ul>
      <br />
      <h2>參考資料：</h2>
      <ul>
